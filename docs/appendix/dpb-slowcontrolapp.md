...
The commands are exposed by a Linux service implemented in the DPB board that sits on top of the protocol
above (`DIG0` / `DIG1`). Each service command maps to one or more `HKDIG_*` write commands from the tables above.

`dpb-slowcontrolapp` commands consist if a `READ/SET` command, followed by Digitizer selection `DIG0/DIG1` and then by the _Service command_ shown hereafter. The following table maps the write commands of the two protocols:
...
> decoding the returned values in the proper way.
